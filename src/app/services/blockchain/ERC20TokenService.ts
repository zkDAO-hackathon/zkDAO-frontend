...
import { Address, WalletClient, createPublicClient, getContract, http, formatUnits } from "viem";
...
const RPCS = {
	...
	avalanche: "https://endpoints.omniatech.io/v1/avax/fuji/public",
} as const;
...
export class ERC20TokenService {
	...
	async getBalance(tokenKey: string, user: Address): Promise<string> {
		...
		try {
			...
			const [balance, decimals] = await Promise.all([contract.read.balanceOf([user]), contract.read.decimals()]);
	...
	async getAllowance(tokenKey: string, owner: Address, spender: Address): Promise<string> {
		const token = this.getTokenData(tokenKey);
		if (!token) return "0";

		try {
			const contract = this.getContract(token.address as Address);
			const [allowance, decimals] = await Promise.all([contract.read.allowance([owner, spender]), contract.read.decimals()]);
	...
	async getTokenInfo(tokenKey: string, user: Address): Promise<TokenInfo | null> {
		const token = this.getTokenData(tokenKey);
